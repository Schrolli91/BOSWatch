	...
	try:
		if configHandler.checkConfig("MySQL"): #read and debug the config
			...
			try:
				...
				connection = mysql.connector.connect(host = globalVars.config.get("MySQL","dbserver"), host = globalVars.config.get("port","dbport"), user = globalVars.config.get("MySQL","dbuser"), passwd = globalVars.config.get("MySQL","dbpassword"), db = globalVars.config.get("MySQL","database"), charset='utf8')
